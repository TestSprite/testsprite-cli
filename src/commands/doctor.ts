...
function checkEndpoint(apiUrl: string): DoctorCheck {
  try { assertValidEndpointUrl(apiUrl); return { name: 'API endpoint', status: 'ok', detail: apiUrl }; }
  catch { return { name: 'API endpoint', status: 'fail', detail: `"${apiUrl}" is not a valid http(s) URL` }; }
}
